...
@dag(dag_id='war_dag_4', schedule_interval=None, tags=['war_dag_4'], render_template_as_native_obj=True, max_active_tasks=1, default_args=default_args)
def war_dag_test():
    ...
    def process_pending_withdrawals(conn, battle_date: datetime):
        ...
        try:
            ...
            for withdrawal_id, account_id, amount in pending_withdrawals:
                ...
                allocation_query = """
                SELECT allocation_id FROM war_iter_4.allocation
                """
    ...
    def liquidate(conn, battle_date):
        """
        Fetches all allocations from war_iter_4.allocation and processes them for removal.
        """
        query = "SELECT * FROM war_iter_4.allocation"
        try:
            with conn.cursor() as cur:
                cur.execute(query)
    ...
    def process_battleday(battle_date, conn):
        ...
        try:
            ...
            battle_date_str = battle_date.strftime("%Y-%m-%d")
            cur.execute(
                "SELECT stocktrader.process_battleday_leads_ml(%s, %s::date)",
                (schema_name, battle_date_str)
            )
    ...
    @task()
    def process_control_flag(date_str):
        conn = PostgresConnection().connect()
        date = datetime.strptime(date_str, "%Y-%m-%d")
        check_control_flag(conn, date)

    @task()
    def process_battleday_logic(date_str):
        conn = PostgresConnection().connect()
        date = datetime.strptime(date_str, "%Y-%m-%d")
        process_battleday(date, conn)

    @task()
    def remove_by_azure(date_str):
        conn = PostgresConnection().connect()
        db_helper = GenericDBHelper(conn)
        date = datetime.strptime(date_str, "%Y-%m-%d")
        remove_allocation_to_sell_using_StockAnalyzerUsingAzureAPI(db_helper, conn, date)

    @task()
    def process_assigned_units(date):
        conn = PostgresConnection().connect()




        assigned_units = get_units(conn=conn, fetch='assigned')
        num_assigned_units = len(assigned_units)

        print("number of assigned units",num_assigned_units)
        if num_assigned_units <= 0:
            print("No Assigned Unit Found!")
            return

        print("current---------------dates----------", date)
        current_date = datetime.strptime(date, "%Y-%m-%d")
        battle_date = current_date

        check_control_flag(conn, current_date)
        ...
        accounts = set(unit[4] for unit in assigned_units)
    ...
    @task()
    def process_unassigned_units(date):
        conn = PostgresConnection().connect()
        ...
        for unit in total_units:
            ...
            if unit_assignment_status == 'unassigned' and leads_data_from_table and strength_to_allocate_each_unit <= remaining_strength:
    ...
    @task()
    def fill_summary(date_str):
        conn = PostgresConnection().connect()
        filling_summary_table(conn, date_str)


    @task()
    def process_deposit_withdrawal(date_str):
        conn = PostgresConnection().connect()
...
#            for fetched_account_id in [1, 2]:
    ...
    # Initialize the last task of the previous day
    previous_day_group = None

    # Loop over each date string
    for d in date_strings:
        with TaskGroup(group_id=f"day_{d.replace('-', '_')}") as day_group:
            deposit = process_deposit_withdrawal.override(task_id=f"process_deposit_{d}")(d)
            battle = process_battleday_logic.override(task_id=f"process_battle_{d}")(d)
            ctrl = process_control_flag.override(task_id=f"process_ctrl_{d}")(d)
            azure = remove_by_azure.override(task_id=f"process_azure_{d}")(d)
            assigned = process_assigned_units.override(task_id=f"process_units_{d}")(d)
            unassigned = process_unassigned_units.override(task_id=f"process_sell_signals_{d}")(d)
            summary = fill_summary.override(task_id=f"process_summary_{d}")(d)

            deposit >> battle >> ctrl >> azure >> assigned >> unassigned >> summary
            #deposit >> battle >> ctrl >> azure >> assigned >> summary

        # Chain day-to-day TaskGroups
        if previous_day_group:
            previous_day_group >> day_group
